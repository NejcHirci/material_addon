...
sys.path.insert(1, 'materialGAN/PerceptualSimilarity/')
import models
import kornia
...
class Losses:
    def __init__(self, args, textures_init, textures_ref, rendered_ref, res, size, lp, cp):
        ...
        self.lp = lp
        self.cp = cp
        self.criterion = th.nn.MSELoss().cuda() if th.cuda.is_available() else th.nn.MSELoss()
    ...
    def eval_render_jitter(self, textures, li):
        ...
        rendered = th.zeros(self.args.num_render_used, 3, self.res, self.res).cuda() if th.cuda.is_available() else th.zeros(self.args.num_render_used, 3, self.res, self.res)
        for i in range(self.args.num_render_used):
            lp_this = self.lp[i,:]
            light = th.from_numpy(li).cuda() if th.cuda.is_available() else th.from_numpy(li)
            rendered[i,:] = renderOBJ.eval(textures, lightPos=lp_this, cameraPos=self.cp[i,:], light=light)

        renderOBJ_jitter = Microfacet(res=self.res, size=self.size)
        rendered_jitter = th.zeros(self.args.num_render_used, 3, self.res, self.res).cuda() if th.cuda.is_available() else th.zeros(self.args.num_render_used, 3, self.res, self.res)
        for i in range(self.args.num_render_used):
            lp_this = self.lp[i,:] + np.random.randn(*self.lp[i,:].shape) * 0.1
            light = th.from_numpy(li).cuda() if th.cuda.is_available() else th.from_numpy(li)
            rendered_jitter[i,:] = renderOBJ_jitter.eval(textures, lightPos=lp_this, cameraPos=self.cp[i,:], light=light)
