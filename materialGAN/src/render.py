...
class Microfacet:
    ...
    def initGeometry(self):
        tmp = th.arange(self.res, dtype=th.float32).cuda() if th.cuda.is_available() else th.arange(self.res, dtype=th.float32)
    ...
    def getDir(self, pos):
        pos = th.from_numpy(pos).cuda() if th.cuda.is_available() else th.from_numpy(pos)
...
def tex2pngs(tex, fn):
    ...
    if isSpecular:
        ...
        specular.save(os.path.join(fn, 'specular.png'))
    
    albedo.save(os.path.join(fn, 'albedo.png'))
